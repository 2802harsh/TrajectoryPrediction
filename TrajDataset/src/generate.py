...
def generate_group_obstacle(num_ped, sim=None, radius=4, mode=None): 
    positions = []
    goals = []
    speed = []
    agent_list = []
    ...
    for i in range(num_ped-1):
        while True:
            rad = radius + random.uniform(0,r)
            angle = random.uniform(0, 1) * np.pi / 2
            # add some noise to simulate all the possible cases robot could meet with human
            px_noise = (random.uniform(0, 1) - 0.5)  ## human.v_pref
            py_noise = (random.uniform(0, 1) - 0.5)  ## human.v_pref
            px = rad * np.cos(angle) + px_noise
            py = rad * np.sin(angle) + py_noise
    ...
    ix = -cx + random.uniform(0,0.5)
    iy = -cy - random.uniform(0,0.5)
    fx = cx - random.uniform(0,0.5)
    fy = cy + random.uniform(0,0.5)
    positions.append((ix, iy))
    goals.append((fx,fy))
    sim.addAgent((ix,iy))

    velocity = np.array([fx-ix, fy-iy])
    magnitude = np.linalg.norm(velocity)
    init_vel = 1 * velocity / magnitude if magnitude > 1 else velocity
    speed.append([init_vel[0], init_vel[1]])
...
def generate_across_groups(num_ped, sim=None, radius=4, mode=None): 
    positions = []
    goals = []
    speed = []
    agent_list = []
    ...
    for j in range(2):
        for i in range(nped//2):
            while True:
                rad = radius
                if j == 0:
                   angle = random.uniform(0.85, 1) * np.pi / 3
                else:
                   angle = random.uniform(0.85, 1) * (np.pi + np.pi/2.5)
                # add some noise to simulate all the possible cases robot could meet with human
                px_noise = (random.uniform(0, 1) + 0.25)  ## human.v_pref
                py_noise = (random.uniform(0, 1) - 0.25)  ## human.v_pref
                px = rad * np.cos(angle) + px_noise
                py = rad * np.sin(angle) + py_noise
    ...
    ix /= len(positions)//2
    iy /= len(positions)//2
    ix += random.uniform(0.5,slope)
    iy -= random.uniform(0.5,slope)
    ...
    fx /= len(positions)//2
    fy /= len(positions)//2
    fx -= random.uniform(0.5,slope)
    fy += random.uniform(0.5,slope)

    #ix = cx - random.uniform(2,5)
    #iy = cy - random.uniform(5,10)
    #fx = cx + random.uniform(2,5)
    #fy = cy + random.uniform(5,10)
...
def generate_orca_trajectory(sim_scene, num_ped, min_dist=3, react_time=1.5, end_range=1.0, mode=None):
    ...
    elif sim_scene == 'group_obstacle':
        fps = 100
        sampling_rate = fps / 2.5
        sim = rvo2.PyRVOSimulator(1/fps, 10, 10, 5, 5, 0.3, 1)
        if mode == 'trajnet':
            sim = rvo2.PyRVOSimulator(1/fps, 4, 10, 4, 5, 0.6, 1.5) ## (TrajNet++)
        trajectories, _, goals, speed = generate_group_obstacle(num_ped, sim, mode=mode)
    
    elif sim_scene == 'across_groups':
        fps = 100
        sampling_rate = fps / 2.5
        sim = rvo2.PyRVOSimulator(1/fps, 1.5, 5, 2, 5, 0.2, 1)
        if mode == 'trajnet':
            sim = rvo2.PyRVOSimulator(1/fps, 4, 10, 4, 5, 0.6, 1.5) ## (TrajNet++)
        trajectories, _, goals, speed = generate_across_groups(num_ped, sim, mode=mode)
    else:
...
def main():
    ...
    parser.add_argument('--simulation_scene', default='circle_crossing',
                        choices=('across_groups', 'group_obstacle', 'circle_crossing', 'two_ped', 'two_ped_angle'))
